...
import dev.renkinProject.renkin.constants.SuppressDeprecation
...
import dev.renkinProject.renkin.xml.XmlEncoder
...
import dev.renkinProject.renkin.xml.file.BaseInsetXml
import dev.renkinProject.renkin.xml.file.InsetXml
import dev.renkinProject.renkin.xml.file.VectorWrapperXml
import java.io.File
...
class IconPackBuilder(
    ...
    fun buildAndSign(
        ...
        for (app in apps) {
            if (app.createdIcon != null) {
                ...
                // Always write the static <item> mapping (the user's chosen icon), including for
                // calendar apps, so launchers that don't support <calendar> show that icon instead
                // of the app's default. Calendar-supporting launchers still rotate via the
    ...
    private fun createMainActivity(manifest: AndroidManifestBlock) {
        ...
        activityName.valueAsString = "dev.renkinProject.renkinpack.MainActivity"
