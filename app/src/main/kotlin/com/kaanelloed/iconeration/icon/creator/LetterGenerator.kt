...
class LetterGenerator(ctx: Context) {
    ...
    fun generateFirstLetter(appName: String, color: Int, strokeWidth: Float): Drawable {
        var text = appName.trim()
        if (text.isNotEmpty()) {
            text = text.substring(0, 1)
        }

        return TextDrawable(text, font, 200F, color, strokeWidth)
    }

    fun generateTwoLetters(appName: String, color: Int, strokeWidth: Float): Drawable {
        var text = appName.trim()
        ...
        return TextDrawable(text, font, 150F, color, strokeWidth)
    }
