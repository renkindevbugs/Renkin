...
class PathExporter() {
    ...
    private fun nodeToString(node: PathNode): String {
        return when (node) {
            ...
            is PathNode.RelativeArcTo -> "a ${node.horizontalEllipseRadius} ${node.verticalEllipseRadius} ${node.theta} ${node.isMoreThanHalf} ${node.isPositiveArc} ${node.arcStartDx}, ${node.arcStartDy}"
            is PathNode.ArcTo -> "A ${node.horizontalEllipseRadius} ${node.verticalEllipseRadius} ${node.theta} ${node.isMoreThanHalf} ${node.isPositiveArc} ${node.arcStartX}, ${node.arcStartY}"
        }
    }
